...
{
    internal static class StopAfter
    {
        ...
        public static void BotStarted()
        {
            if (LazySettings.StopAfterBool)
            {
                ...
                _stopAfter.Reset();
                Logging.Write(LogType.Info, "Stop after enabled, will stop in " + Convert.ToDouble(LazySettings.StopAfter) +  " minuttes");
            }
        }
            ...
            if (_stopAfter != null)
            {
                if (LazySettings.StopAfterBool)
                {
                    if (_stopAfter.IsReady && !ObjectManager.ShouldDefend)
                    {
                        ...
                        Thread.Sleep(3000);
                        Logging.Write(LogType.Info, "[Engine]Stop after " + Convert.ToDouble(LazySettings.StopAfter) + " minuttes done");
