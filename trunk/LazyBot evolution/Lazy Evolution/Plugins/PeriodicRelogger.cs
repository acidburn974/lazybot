    ...
    internal class PeriodicRelogger
    {
        ...
        public static void BotStarted()
        {
            if (ReloggerSettings.PeriodicReloggingEnabled && ReloggerSettings.ReloggingEnabled)
            {
                ...
                _logOutAfter.Reset();
                Logging.Write(LogType.Info, "Periodic relog enabled. Next logout in: " + Convert.ToDouble(ReloggerSettings.PeriodicLogOut) + " minutes");
            }
        }
            ...
            if (_logOutAfter != null)
            {
                if (ReloggerSettings.PeriodicReloggingEnabled)
                {
                    if (_logOutAfter.IsReady && !ObjectManager.ShouldDefend)
                    {
                        ...
                        Thread.Sleep(3000);
                        Logging.Write(LogType.Info, "[Engine] Periodic logout as " + Convert.ToDouble(ReloggerSettings.PeriodicLogOut) + " minutes have passed");
