...
namespace LazyLib.Wow
{
    ...
    public static class ObjectManager
    {
        ...
        public static PObject GetObjectByGuid(ulong guid)
        {
            lock (Locker)
            {
                return ObjectList.Where(wowObject => wowObject.GUID.Equals(guid)).FirstOrDefault();
            }
        }
        ...
        public static void MakeReady()
        {
            ...
            _monitor = new Thread(Monitor) {IsBackground = true, Name = "ObjectManager"};
            _monitor.Start();
        }
        ...
        private static void ReadObjectList()
        {
            ...
            while (currentObject.BaseAddress != UInt32.MinValue && currentObject.BaseAddress%2 == UInt32.MinValue)
            {
                ...
                currentObject.BaseAddress = Memory.Read<uint>(currentObject.BaseAddress + (uint) Pointers.ObjectManager.NextObject);
            }
        }
            ...
            if (DoesProcessExsist(pid))
            {
                ...
                if (InGame)
                {
                    try
                    {
                        ...
                        _refresher = new Thread(Pulse) {IsBackground = true, Name = "Pulse"};
                        _refresher.Start();
                        if (Attach != null)
                        {
        ...
        public static List<PUnit> CheckForMobsAtLoc(Location l, float radius, bool includeFriendly)
        {
            ...
            if (l != null)
            {
                ...
                {
                    if (includeFriendly)
                        returns.AddRange(mobs.Select(mob => new {mob, mdt = mob.Location.GetDistanceTo(l)}).Where(
                            @t => @t.mdt <= radius && !@t.mob.IsDead && !@t.mob.IsTagged && @t.mob.Level > 1 &&
                                  !@t.mob.IsTargetingMe).Select(@t => @t.mob));
                    else
                        returns.AddRange(mobs.Select(mob => new {mob, mdt = mob.Location.GetDistanceTo(l)}).Where(
                            @t => @t.mdt <= radius && !@t.mob.IsDead && !@t.mob.IsTagged && @t.mob.Level > 1 &&
                                  !@t.mob.IsTargetingMe && @t.mob.Reaction.Equals(Reaction.Hostile)).Select(@t => @t.mob));
                }
            }
            return returns;
        }
